...
exports.assignModule = asyncHandler(async (req, res, next) => {
  ...
  if (mod.status !== "published")
    return next(
      new ErrorResponse("Only published modules can be assigned", 400),
    );
  ...
  if (accessRules.length > 0) {
    ...
    if (!openToAll) {
      ...
      const uniqueRoleNames = [
        ...new Set(learnerUsers.map((u) => u.role).filter(Boolean)),
      ];
      ...
      for (const u of learnerUsers) {
        learnerRoleIdMap[String(u._id)] =
          roleNameToId[u.role?.toLowerCase()] || null;
      }
      ...
      if (eligibleIds.length === 0) {
        return res.status(403).json({
          success: false,
          message:
            "None of the learners have a role permitted by this module's access rules.",
...
exports.getMyAssignments = asyncHandler(async (req, res, next) => {
  const assignments = await ModuleAssignment.find({ learner: req.user._id })
    .populate({
      ...
      populate: {
        path: "parts",
        model: "TrainingModulePart",
        options: { sort: { order: 1 } },
        populate: {
          path: "questions",
          model: "TrainingModuleQuestion",
        },
      },
    })
...
exports.getAssignment = asyncHandler(async (req, res, next) => {
  ...
  if (!isOwner)
    return next(
      new ErrorResponse("Not authorised to view this assignment", 403),
    );
...
exports.deleteAssignment = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorResponse("Not authorised to revoke this assignment", 403),
    );
  }
